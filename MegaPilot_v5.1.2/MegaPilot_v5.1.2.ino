...
unsigned long lastToggleTime[4] = {0, 0, 0, 0};
...
int currentStep = 0;
bool isSequentialToggleRunning = false;
const int* currentIndices = nullptr;
int currentSize = 0;
bool currentState = false;
const uint8_t* currentOutputPins = nullptr;
...
#ifdef __arm__
// Для ARM-архитектуры (например, Arduino Due)
extern "C" char* sbrk(int incr);
#else  // __arm__
// Для AVR-архитектуры (например, Arduino Uno)
...
void sequentialToggle(const int* indices, int size, bool state, const uint8_t* outputPins) {
  if (!isSequentialToggleRunning) {
    currentIndices = indices;
    currentSize = size;
    currentState = state;
    currentOutputPins = outputPins;
    currentStep = 0;
    isSequentialToggleRunning = true;
    previousMillis = millis();
    checkScript();
  }
}

void updateSequentialToggle() {
  if (isSequentialToggleRunning && millis() - previousMillis >= intervalClick) {
    if (currentStep < currentSize) {
      uint8_t pinIndex = currentIndices[currentStep];
      
      for (int j = 0; j < 16; j++) {
        if (OUTPUT_PINS1[j] == pinIndex) {
          outputState[j] = currentState;
          break;
        }
      }
      mcp1.digitalWrite(currentOutputPins[currentStep], currentState);
      Serial.print("Pin ");
      ...
      isSequentialToggleRunning = false;
      Serial.println("Sequential toggle finished");
      checkScript();
    }
  }
}
...
void handleButtonHold(int index, int blinkCount) {
    if (btns[index].hold()) {
        blinkLamp(blinkCount);
        functionState[index] = !functionState[index];
        mcp3.digitalWrite(OUTPUT_PINS3[index], functionState[index]);
        Serial.print("\t Function ");
        Serial.print(index);
        Serial.print(": ");
        Serial.println(!functionState[index]);
        btns[index].clear();
        lastToggleTime[index] = millis();
    }
    
    if (lastToggleTime[index] > 0 && millis() - lastToggleTime[index] >= intervalHold0) {
        functionState[index] = !functionState[index];
        mcp3.digitalWrite(OUTPUT_PINS3[index], functionState[index]);
        Serial.print("\t Function ");
        Serial.print(index);
        Serial.print(": ");
        Serial.println(!functionState[index]);
        lastToggleTime[index] = 0; // Сброс таймера
    }
}
...
void setup() {
  ...
  Serial.println("Start MegaPilot_v5.1.2");
  blinkLamp(5);
  printAllValues();
...
void loop() {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    handleButtonHold(0, 1);

    handleButtonHold(3, 3);
