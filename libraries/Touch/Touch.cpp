                     ...
                     uint16_t hwXpos3, uint16_t hwYpos3, uint16_t hwFac)
{
	...
	Tft.lcd_display_string(40, 40, (const uint8_t *)"ArduBlock.ru",16, RED);
	Tft.lcd_display_string(40, 55, (const uint8_t *)"PS Vitaliy Petrov",16, BLACK);
	Tft.lcd_display_string(40, 70, (const uint8_t *)"Touch Calibration!",16, RED);
}
		...
		if((s_tTouch.chStatus & 0xC0) == TP_PRESSED) {	
			...
			switch(cnt) {			   
				...
				case 4:	
					...
					Tft.lcd_clear_screen(WHITE);
					Tft.lcd_display_string(35, 110, (const uint8_t *)"Touch OK!", 16, BLUE);
					delay(1000); 
 					Tft.lcd_clear_screen(WHITE);  
					return;				 
			}
...
s_tTouch.hwXpos = s_tTouch.hwXpos / 2;
s_tTouch.hwYpos = s_tTouch.hwYpos / 2;
if (s_tTouch.hwXpos > 250){
s_tTouch.hwXpos = 0;
}
if (s_tTouch.hwYpos > 250){
s_tTouch.hwYpos = 0;
}
EEPROM.write(250, s_tTouch.hwXpos);
EEPROM.write(251, s_tTouch.hwYpos);
tp_scan(0);
